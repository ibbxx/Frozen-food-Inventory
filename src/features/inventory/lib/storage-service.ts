import { inventorySupabase } from "./supabase";
...
export async function uploadProductImage(file: File): Promise<string> {
  ...
  const { data, error } = await inventorySupabase.storage
  ...
  const { data: publicUrlData } = inventorySupabase.storage
    .from(PRODUCT_IMAGES_BUCKET)
...
export async function deleteProductImageFromStorage(
  ...
  try {
    const { error } = await inventorySupabase.storage
      .from(PRODUCT_IMAGES_BUCKET)
      .remove([storagePath]);
